...
export const actions = {
	default: async ({ request }) => {
		...
		try {
			await db.execute({
				sql: `INSERT INTO patients (no_rm, nama, alamat, no_telepon, tanggal_lahir, nomor_identitas, usia) 
				      VALUES (?, ?, ?, ?, ?, ?, ?)`,
				args: [
					no_rm,
					nama,
			...
			throw redirect(303, '/dashboard?success=Data pasien berhasil ditambahkan');
		} catch (error: any) {
